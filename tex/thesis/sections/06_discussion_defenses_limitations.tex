...
\noindent\textit{\footnotesize Code excerpt: Defense guard between retrieval and reranking \cite{ragpoisonlab_repo}.}\par\vspace{0.2em}
\begin{minted}{python}
active_defense = (
    defense_config_override
    if mode == "attacked"
    and defense_config_override is not None
    and defense_config_override.enabled
    else None
)

defense_result = apply_retrieval_defense(
    candidates=retrieval_result.candidates,
    config=active_defense,
)
retrieval_candidates = list(defense_result.candidates)

prompt_candidates, prompt_defense_debug = sanitize_candidates_for_prompt(
